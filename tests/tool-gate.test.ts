...
function test8_supremeCoordinatorRules(): void {
  ...
  // Action-level blocks: idumb_init (install), govern_task (quick_start/start/complete/fail/review)
  assert("supreme-coordinator blocks install on idumb_init", rules.blockedActions["idumb_init"]?.has("install") === true)
  assert("supreme-coordinator blocks quick_start on govern_task", rules.blockedActions["govern_task"]?.has("quick_start") === true)
