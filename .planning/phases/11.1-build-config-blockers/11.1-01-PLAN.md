...
<task type="auto">
  ...
  <action>
    ...
    **Note on GOV-3 assertion shape:** Verify the actual SDK return type from `session.messages()` before applying the assertion. If it returns `Message[]` rather than `Array<{ info: Message; parts: Part[] }>`, use `as Message[]` instead. The `npm run typecheck:app` verification step will catch any mismatch.
  </action>
  <verify>
...
<task type="auto">
  ...
  <action>
    **DRIFT-2: Update test baselines in CLAUDE.md and AGENTS.md**
    ...
    In `AGENTS.md`, update all three test baseline references:
    ...
    - Change `must pass baseline suites (10 suites, 512 assertions)` to `must pass baseline suites (10 suites, 591 assertions)` (in Session Handoff section, line ~515)
