...
must_haves:
  truths:
    ...
    - "Appearance tab shows theme toggle (light/dark/system)"
    - "AI tab shows configured providers with model count and default model/agent selection"
    - "Connection tab shows engine status (URL, port, project dir) and health"
  ...
  key_links:
    ...
    - from: "pages/SettingsPage.tsx Connection tab"
      to: "hooks/useEngine.ts → useEngineStatus()"
      via: "React Query hook"
...
Purpose: Users need a central place to view and configure the app. The sidebar indicator needs to convey more than "connected/disconnected" — it should show provider count and health semantics.
...
<task type="auto">
  ...
  **1. `src/dashboard/frontend/src/pages/SettingsPage.tsx`** — Create tabbed settings page.

  Use plain Tailwind tab pattern (no shadcn Tabs component — not installed). Pattern: button row with active state, conditional content rendering.
  ...
  **AppearanceTab** — Theme toggle. Read current theme from `document.documentElement.classList` or localStorage. Three buttons: Light, Dark, System.
  ...
  **AITab** — Show configured providers with model count. Uses `useProviders()` and `useAgents()` from hooks/useConfig.ts (created in plan 01-09).
  ...
  **ConnectionTab** — Show engine status using `useEngineStatus()`.
  ...
  **GovernanceTab** — Show current governance mode (readonly for now — mode changes require plugin config).
  ...
  **2. `src/dashboard/frontend/src/App.tsx`** — Add the /settings route.
  ...
  Note: Sidebar.tsx already has a `<NavLink to="/settings">` — it just pointed to a nonexistent route. Adding this route makes it functional.
  </action>
  <verify>
  ...
  3. `grep "SettingsPage" src/dashboard/frontend/src/App.tsx` — route wired
  4. `grep 'path="settings"' src/dashboard/frontend/src/App.tsx` — route path exists
  5. `grep "useProviders" src/dashboard/frontend/src/pages/SettingsPage.tsx` — hook used
  6. `grep "useEngineStatus" src/dashboard/frontend/src/pages/SettingsPage.tsx` — hook used
  </verify>
  <done>
...
<task type="auto">
  ...
  **Health semantics:**
  - 🟢 Green: Connected AND at least 1 provider configured
  - 🟡 Yellow: Connected BUT no providers configured (user needs to set up API keys)
  - 🔴 Red: Disconnected
  ...
  <verify>
  1. `cd src/dashboard/frontend && npx tsc --noEmit` — zero TypeScript errors
  2. `grep "useProviders" src/dashboard/frontend/src/components/layout/Sidebar.tsx` — import exists
  3. `grep "providerCount" src/dashboard/frontend/src/components/layout/Sidebar.tsx` — health logic exists
  4. `grep "bg-yellow-500" src/dashboard/frontend/src/components/layout/Sidebar.tsx` — yellow state for no-providers
  </verify>
  <done>
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <action>
  Human visual verification of config UI. No code changes — verify what was built in Tasks 1-2.
  ...
  2. Navigate to `/settings` — verify 4 tabs render, Settings link in sidebar works
  3. Check AI tab — should list providers and agents from OpenCode
  4. Check Connection tab — should show engine URL, port, project dir
  5. Open a chat session — verify model selector and agent selector appear in session header bar
  6. Select a model, refresh page — verify selection persists
  7. Check sidebar footer — should show provider/model count below connection dot

  Resume signal: Type "approved" or describe issues
  </action>
  <verify>User confirms visual/functional correctness of all config UI components</verify>
  <done>User typed "approved" — config UI verified working</done>
</task>
...
3. **Test baseline**: `npm test` — all existing tests pass (no test regressions)
