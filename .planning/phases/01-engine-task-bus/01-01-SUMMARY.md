...
3. **Task 3: SDK proxy result hardening** - `3a570c5` (fix)

## Verification

- `npm run -s typecheck` ✅
- Backend exposes session/engine routes with structured errors.
- SSE endpoints emit JSON events (`/api/events`, `/api/sessions/:id/prompt`) and close cleanly.

## Deviations from Plan

- Used both `OPENCOD_PORT` (plan spelling) and `OPENCODE_PORT` env fallback for compatibility.
- Health check uses SDK `config.get` retries instead of `global.health` (not available in current SDK surface).
