...
**Important:** Dynamic imports for xterm.js are required because it uses DOM APIs that don't exist during SSR. Use `typeof window !== 'undefined'` guard.
  ...
  <verify>
`npm run typecheck:app` passes. `@xterm/xterm`, `@xterm/addon-fit`, `@xterm/addon-attach` are in package.json dependencies. All server functions exist with proper Zod validators. useTerminal hook compiles without type errors.
  </verify>
  <done>
