...
<task type="auto">
  ...
  <action>
    **1. Update root package.json:**
    - Add to dependencies: @tanstack/react-start ^1.159.5, @tanstack/react-router ^1.159.5, react ^18.2.0, react-dom ^18.2.0, @tanstack/react-query ^5.17.19, lucide-react ^0.344.0, react-markdown ^9.0.1, rehype-highlight ^7.0.0, rehype-raw ^7.0.0, remark-gfm ^4.0.0, clsx ^2.1.0, tailwind-merge ^2.2.1, drizzle-orm ^0.45.1, zod ^3.24.0
    - CRITICAL: `zod` is required — Plans 02 and 03 use Zod schemas for server function input validation. Per TanStack Start best practices (sf-input-validation, CRITICAL priority): all server function validators MUST use Zod, not inline type assertions.
    ...
    **4. Create TanStack Start entry files:**
    ...
    `app/start.ts` — Global middleware config (registers request + server function middleware):
    ```typescript
    import { createStart } from '@tanstack/react-start'

    // CRITICAL: createStart takes a CALLBACK returning config, not a plain object.
    // Export as named const (per TanStack Start docs), not default export.
    // CORS is NOT needed — SPA mode is same-origin. This file is for
    // registering global request middleware and server function middleware.
    export const startInstance = createStart(() => {
      return {
        // functionMiddleware: [],  // Add server function middleware here (Plan 02)
        // requestMiddleware: [],   // Add request middleware here (auth, logging)
      }
    })
    ```
    NOTE: The file MUST be at `app/start.ts` (or `src/start.ts` if srcDirectory is `src/`). TanStack Start auto-discovers this file. See context7 docs for `createStart` API.
