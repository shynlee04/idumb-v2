...
<task type="auto">
  ...
  <action>
    ...
    **4. Create app/db/index.server.ts — database client singleton:**
    NAMING: Use `.server.ts` suffix to prevent client-side import. `better-sqlite3` is a native
    Node.js module — if any client component accidentally imports from `app/db/`, it will pull
    the native binding into the browser bundle, causing a build failure or runtime crash.
    TanStack Start tree-shakes `.server.ts` files from the client bundle automatically.
    ```typescript
    ...
    IMPORTANT: All imports of this file MUST use `'@/db/index.server'` or `'../db/index.server'` (with `.server` suffix).
...
<task type="auto">
  ...
  <action>
    **1. Create app/server/settings.ts — settings CRUD server functions:**
    ...
    // --- Zod Validators (sf-input-validation: CRITICAL) ---
    // Per TanStack Start best practices: inline type assertions like
    // `.validator((d: { key: string }) => d)` provide ZERO runtime validation.
    // They are identity functions that pass everything through.
    // Zod schemas provide actual runtime parsing + type safety.
    const KeySchema = z.object({ key: z.string().min(1) })
    const KeyValueSchema = z.object({ key: z.string().min(1), value: z.string() })
    ...
    **2. Verify persistence round-trip:**
    ...
    ⚠️ WARNING: This script uses `console.log` which violates AGENTS.md rule "NO console.log anywhere."
    This is ACCEPTABLE ONLY because: (1) it's a temporary throwaway script, (2) it runs outside
    the TUI context, (3) it MUST be deleted immediately after verification. Do NOT commit this file.
    ```typescript
    import { db } from '../app/db/index.server.ts'
  ...
  <verify>
    ...
    - All validators use Zod schemas (KeySchema, KeyValueSchema) — NO inline type assertions
    - app/db/index.server.ts exists (NOT index.ts) — prevents client-side import of better-sqlite3
    - Settings imports use `@/db/index.server` path (with .server suffix)
  </verify>
  <done>
...
7. All server function validators use Zod schemas — `grep -r "validator((d:" app/` returns ZERO hits
8. Database client is at `app/db/index.server.ts` — `ls app/db/index.ts` should NOT exist
9. No `import.*from.*app/db` in any file under `app/routes/` or `app/components/` (client code)
</verification>
