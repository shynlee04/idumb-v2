...
<task type="auto">
  ...
  <action>
    **1. Create app/server/sdk-client.ts — SDK client singleton:**
    ...
    **2. Create app/server/engine.ts — engine lifecycle server functions:**
    ...
    import { ensureEngine, getEngineStatus, stopEngine } from './sdk-client'
    ...
    **3. Create app/server/sessions.ts — session CRUD server functions:**
    ...
    - `createSessionFn` (POST, validator: `{ name?: string }`): calls `getClient().session.create()`, returns `{ session: Session }`
    - `getSessionFn` (GET, validator: `{ sessionId: string }`): calls `getClient().session.get(sessionId)`, returns session object
    - `deleteSessionFn` (POST, validator: `{ sessionId: string }`): calls `getClient().session.delete(sessionId)`, returns `{ success: true }`
    - `getSessionMessagesFn` (GET, validator: `{ sessionId: string }`): calls `getClient().session.messages(sessionId)`, returns `{ messages }`
    - `abortSessionFn` (POST, validator: `{ sessionId: string }`): calls `getClient().session.abort(sessionId)`, returns `{ success: true }`
    - `getSessionStatusFn` (GET, validator: `{ sessionId: string }`): returns session's current status
    - `getSessionChildrenFn` (GET, validator: `{ sessionId: string }`): returns child sessions
  ...
  <verify>
    - All 4 files exist in app/server/
    ...
    - sdk-client.ts exports getClient, ensureEngine, getEngineStatus, stopEngine
