...
**1. Replace hardcoded defaultSize with persisted store values:**

Currently each Panel uses `defaultSize={IDE_PANELS.sidebar.defaultSize}` (hardcoded 20/50/30).
Change to read from the persisted store on mount:
...
Replace:
- `defaultSize={IDE_PANELS.sidebar.defaultSize}` → `defaultSize={panelSizes.sidebar}`
- `defaultSize={IDE_PANELS.editor.defaultSize}` → `defaultSize={panelSizes.editor}`
- `defaultSize={IDE_PANELS.terminal.defaultSize}` → `defaultSize={panelSizes.terminal}`

**2. Add collapse state restoration on mount:**

Use `usePanelRef` (already imported from react-resizable-panels) to get refs for sidebar and terminal panels. Add a `useEffect` that runs once on mount to restore collapsed state:
...
useEffect(() => {
  // Restore collapsed state from persisted store
  if (collapsed.includes('sidebar')) {
    sidebarRef.current?.collapse()
  }
  if (collapsed.includes('terminal')) {
    terminalRef.current?.collapse()
  }
}, []) // Only on mount — collapsed is from initial store state
```

Pass `panelRef={sidebarRef}` and `panelRef={terminalRef}` to the respective Panel components.
...
- The `defaultSize` prop is only read on initial mount — this is expected React behavior
- Import `usePanelRef` from `react-resizable-panels` (it's already available in the installed version)
  </action>
  <verify>
