...
must_haves:
  ...
  key_links:
    ...
    - from: "app/components/ide/IDEShell.tsx"
      ...
      via: "renders EditorArea in editor panel (replacing EditorPlaceholder)"
...
These files from 06-01 and 06-02 MUST exist (actual paths):
- app/stores/ide-store.ts (Zustand store with openTabs, activeTabId, markDirty, markClean, saveViewState, closeTab, setActiveTab — created in 06-02)
- app/stores/layout-store.ts (Zustand store for panel layout — created in 06-01, NOT modified here)
- app/server/files.ts (readFile, writeFile server functions — created in 06-02)
- app/shared/ide-types.ts (Tab type — path, name, isDirty — added in 06-02)
- app/components/ide/IDEShell.tsx (three-panel layout, sidebar has FileTree from 06-02, editor still has EditorPlaceholder)
- app/hooks/useFiles.ts (useFileContent hook for fetching file content — created in 06-02)
...
<task type="auto">
  ...
  1. Create `app/lib/monaco-workers.ts` — Monaco Web Worker configuration (research Example 7):
     ...
     This MUST be imported before any Monaco usage.
  ...
  3. Create `app/components/editor/MonacoEditor.lazy.tsx` — SSR-safe lazy wrapper (pitfall P1 — CRITICAL):
     ...
     This prevents Monaco from being evaluated during TanStack Start shell prerender.
...
<task type="auto">
  <name>Task 2: TabBar and EditorArea components, wire into IDEShell editor panel</name>
  <files>app/components/editor/TabBar.tsx, app/components/editor/EditorArea.tsx, app/components/ide/IDEShell.tsx</files>
  <action>
  1. Create `app/components/editor/TabBar.tsx` — tab bar with active highlight, dirty indicator, close:
     ...
     Key details:
     - `overflow-x-auto scrollbar-hide` for tab overflow
     - Blue dot dirty indicator shown when `tab.isDirty`
     - Close button revealed on hover via `opacity-0 group-hover:opacity-100`
  ...
  2. Create `app/components/editor/EditorArea.tsx` — vertical layout combining TabBar + MonacoEditor:
     ...
     Key: `min-h-0` on the editor container is CRITICAL for flexbox.

  3. Modify `app/components/ide/IDEShell.tsx` — replace EditorPlaceholder with EditorArea:

     - Import `EditorArea` from `../editor/EditorArea`

     Replace the `EditorPlaceholder` component AND its usage in the editor Panel with:
     ```tsx
     <EditorArea />
     ```

     Keep TerminalPlaceholder unchanged (Phase 7).
  ...
  - `grep "EditorArea" app/components/ide/IDEShell.tsx` — imported and rendered in editor panel
  - `grep "EditorPlaceholder" app/components/ide/IDEShell.tsx` — should NOT appear (replaced)
  - `npx tsc --noEmit 2>&1 | head -20` — quick typecheck, fix any errors
  </verify>
  <done>Tab bar with active highlighting, dirty indicators, close buttons, and overflow scroll. EditorArea combines TabBar + LazyMonacoEditor with empty state. Editor panel in IDEShell replaced with EditorArea. Full end-to-end flow: click file → tab opens → edit → save → close.</done>
</task>
...
- No console errors about Web Workers
...
4. SSR-safe: typecheck passes, no "window is not defined" errors
