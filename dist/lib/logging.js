 ...
 */
export function getLogPath(directory) {
 ...
 */
export function log(directory, level, message, data) {
    if (!shouldLog(level))
        return;
    try {
 ...
 */
export function debug(directory, message, data) {
    log(directory, "debug", message, data);
}
export function info(directory, message, data) {
    log(directory, "info", message, data);
}
export function warn(directory, message, data) {
    log(directory, "warn", message, data);
}
export function error(directory, message, data) {
    log(directory, "error", message, data);
}
/**
 * Log with explicit path (for cases where directory context is unavailable)
 */
export function logToPath(logPath, level, message, data) {
    if (!shouldLog(level))
        return;
    try {
        ensureLogDir(logPath);
 ...
 */
export function createLogger(directory, module) {
    return {
        debug: (message, data) => debug(directory, `[${module}] ${message}`, data),
        info: (message, data) => info(directory, `[${module}] ${message}`, data),
        warn: (message, data) => warn(directory, `[${module}] ${message}`, data),
        error: (message, data) => error(directory, `[${module}] ${message}`, data),
    };
}
/**
 * Clear log file (useful for testing)
 */
export function clearLog(directory) {
    try {
        const logPath = getLogPath(directory);
