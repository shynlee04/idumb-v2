 ...
 */
function walkDirectory(directory, maxDepth = 8) {
    const stats = {
        sourceFiles: [],
        testFiles: [],
        configFiles: [],
...
function detectStack(directory, pkg) {
    const stack = [];
    if (pkg) {
    ...
    if (existsSync(join(directory, "go.mod")))
        stack.push("go-modules");
    if (existsSync(join(directory, "requirements.txt")))
        stack.push("pip");
    if (existsSync(join(directory, "pyproject.toml")))
        stack.push("python-project");
    if (existsSync(join(directory, "Gemfile")))
        stack.push("ruby-bundler");
...
function detectPackageManager(directory) {
    ...
    if (existsSync(join(directory, "package-lock.json")))
        return "npm";
    if (existsSync(join(directory, "Cargo.lock")))
        return "cargo";
    if (existsSync(join(directory, "go.sum")))
        return "go";
    if (existsSync(join(directory, "requirements.txt")))
        return "pip";
    if (existsSync(join(directory, "Gemfile.lock")))
        return "bundler";
    if (existsSync(join(directory, "composer.lock")))
        return "composer";
    return null;
}
// ============================================================================
...
// ============================================================================
function classifyStage(sourceCount, testCount, configCount) {
    if (sourceCount < 5)
        return "greenfield";
    if (sourceCount > 50 && testCount > 5 && configCount > 3)
        return "mature";
    return "brownfield";
}
// ============================================================================
...
// ============================================================================
function detectConventions(directory, configFiles) {
    const conventions = {
        linting: null,
        formatting: null,
        testing: null,
    ...
    else if (configSet.has(".eslintrc") || configSet.has(".eslintrc.js") ||
    ...
    else if (configSet.has(".prettierrc") || configSet.has(".prettierrc.js") ||
    ...
    if (configSet.has("vitest.config.ts") || configSet.has("vitest.config.js")) {
        ...
        if (pkg?.scripts) {
            ...
            if (scripts.includes("vitest"))
                conventions.testing = "vitest";
            else if (scripts.includes("jest"))
                conventions.testing = "jest";
            else if (scripts.includes("mocha"))
                conventions.testing = "mocha";
            else if (scripts.includes("pytest"))
...
// GAP ANALYSIS
// ============================================================================
function analyzeGaps(files, conventions, directory) {
    const gaps = [];
    let gapId = 0;
    const nextId = () => `gap-${++gapId}`;
    // No tests
...
// DEBT DETECTION
// ============================================================================
function detectDebt(files, directory) {
    const debt = [];
    let debtId = 0;
    const nextId = () => `debt-${++debtId}`;
...
// CONCERN ANALYSIS
// ============================================================================
function analyzeConcerns(files, _directory) {
    const concerns = [];
    let concernId = 0;
    const nextId = () => `concern-${++concernId}`;
    // Very deep nesting
...
// DRIFT DETECTION
// ============================================================================
function detectDrift(directory) {
    const indicators = [];
 ...
 */
export function scanCodebase(directory) {
