...
**Coverage:** All 10 registered tools with inputs, outputs, edge cases, and integration status
...
## Tool 7: `idumb_status` — System State
...
### Registered in `src/index.ts` (main plugin):
1. `idumb_task`
2. `idumb_write` (through `idumb_anchor` — needs verification)
3. `idumb_init`
4. `idumb_scan`
5. `idumb_codemap`

### Registered in `src/tools-plugin.ts` (additional tools):
6. `idumb_bash`
7. `idumb_read`
8. `idumb_webfetch`

### Also registered (from anchor/status):
9. `idumb_anchor`
10. `idumb_status`
...
idumb_status ──→ StateManager ──→ summary

idumb_bash ──→ (standalone, TUI-safe)
idumb_read ──→ (standalone, token-aware)
idumb_webfetch ──→ (standalone, TUI-safe)
```
