...
**Total Items:** 50+ (8 Critical, 16 High, 14 Medium, 12 Low)
...
|----|--------------|-------------|---------|----------|
| MC-1 | "Tests pass (294/294 assertions)" | AGENTS.md v6.0.0 | Tests are standalone `tsx` scripts run via `npm test`. They use custom `assert()` + `process.exit()`. NOT vitest (vitest not in project deps). The 294/294 claim likely from `tsx` runs but UNVERIFIED. Real issue: no CI, no coverage, non-standard runner. | � HIGH |
...
| MC-5 | "Delegation depth tracking" | gap-analysis.md | delegation.ts has getDelegationDepth(). tool-gate.ts does NOT call it. tool-gate HAS `AGENT_TOOL_RULES` but with OLD 7-agent names. | 🟡 HIGH |
...
| MC-11 | "5 hooks registered" | Multiple docs | Actual: 7 hook registrations in index.ts (event, tool.execute.before/after, exp.session.compacting, exp.chat.system.transform, exp.chat.messages.transform, chat.params). SDK supports 13 total. | 🟡 HIGH |
| MC-12 | "system.prompt.transform" hook name | Multiple docs | Actual SDK name: `experimental.chat.system.transform`. Wrong name would cause a fresh agent to misunderstand the codebase. | 🔴 CRITICAL |
| MC-13 | "idumb_status is a registered tool" | Docs 07, 08 | `tools/status.ts` (83 LOC) exists but NOT registered in either plugin entry point. Dead code. Absorbed into `idumb_task action=status`. | 🟡 HIGH |
| MC-14 | "10 tools available" | Doc 07 | Actual: 9 registered tools across two plugins (5 in Plugin A, 4 in Plugin B). | 🟡 HIGH |
| MC-15 | "DELEGATION_EXPIRY_MS = 4 hours" | Doc 09 original | Actual: `30 * 60 * 1000` = 30 minutes. | 🟡 MEDIUM |
| MC-16 | "No runtime permission enforcement" | Docs 01, 09 | tool-gate.ts HAS `AGENT_TOOL_RULES` with `blockedTools`/`blockedActions` for 7 agents. But uses OLD names. Partially true, partially false. | 🟡 HIGH |
...
|----|-------|-------------|--------|------------|
| BC-1 | Entity Resolver → Tool Gate | Tool gate never calls resolveEntity(). BUT tool-gate HAS AGENT_TOOL_RULES (stale names) | Permissions partially enforced with wrong agent names | MEDIUM |
...
1. Update AGENT_TOOL_RULES in tool-gate.ts to use 3-agent model names (MC-5, MC-16)
...
5. Fix hook names in all documentation (MC-12)
6. Register or remove idumb_status (MC-13)
7. Fix version mismatch: align package.json and src/index.ts VERSION constant
