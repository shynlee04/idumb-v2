...
| References `tools/anchor.ts` only | 193 | 11 tool files exist in src/tools/ but only 9 are registered across 2 plugins. `idumb_status` is dead code (absorbed into `idumb_task action=status`). |
| Tool names: `idumb_anchor_add`, `idumb_anchor_list`, `idumb_status` | 52-54 | Tools renamed: `idumb_anchor`, `idumb_task`, `idumb_write`, etc. `idumb_status` is dead code — NOT registered in either plugin entry point. |
...
1. **IMMEDIATE:** Verify tests pass via `npm test` (tsx runner). Optionally migrate to vitest for CI/coverage.
