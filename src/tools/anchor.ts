...
export const idumb_anchor_add = tool({
  description:
    "Add a context anchor that survives session compaction. Use this to preserve critical decisions, context, or checkpoints across long sessions.",
  args: {
    type: tool.schema
  ...
  async execute(args, context) {
    const anchor = createAnchor(
      args.type as AnchorType,
      args.content,
      args.priority as AnchorPriority
    )
...
export const idumb_anchor_list = tool({
  description:
    "List all active context anchors with staleness info. Shows what context will survive the next compaction.",
  args: {},
  async execute(_args, context) {
    ...
    const lines: string[] = [`Anchors: ${anchors.length} total`, ""]
    ...
    for (const anchor of anchors) {
      ...
      lines.push(
        `[${anchor.priority.toUpperCase()}] ${anchor.type}: ${anchor.content.substring(0, 80)}${anchor.content.length > 80 ? "..." : ""}`,
        `  ID: ${anchor.id} | Age: ${hours}h${staleTag}`,
        ""
      )
    }
