 ...
 */
export const ROLE_PERMISSIONS: Record<AgentRole, ToolCategory[]> = {
...
export function detectAgentRole(agentName: string): AgentRole {
  ...
  if (name.includes("mid") || name.includes("executor")) return "mid-coordinator"
  if (name.includes("validator") || name.includes("checker")) return "validator"
